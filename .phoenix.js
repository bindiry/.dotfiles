...
function setMousePositionCenterForWindow(window) {
  ...
  Phoenix.log(
    String.format(
      "move mouse to pos for {0} x: {1}, y: {2}",
      window.app().name(),
      pos.x,
      pos.y
    )
...
Key.on("e", mash, function () {
  callApp("CotEditor");
});
Key.on("f", mash, function () {
  callApp("Firefox");
});
Key.on("s", mash, function () {
  callApp("Sequel Pro");
});
Key.on("w", mash, function () {
  callApp("Potato Chat");
});
...
Key.on("up", mashCtrl, function () {
  layoutWindow(0, 0, 1, 0.5);
});
Key.on("down", mashCtrl, function () {
